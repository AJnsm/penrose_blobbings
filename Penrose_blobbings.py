...
import random
import cairosvg
# based on: https://github.com/xnx/penrose/blob/master/penrose.py
...
class RobinsonTriangle:
    ...
    def get_arc_d(self, U, V, W, r_scale=1, half_arc=False):
        """
    ...
    def arcs(self, half_arc=False):
        ...
        arc1_d = self.get_arc_d(self.A, self.B, D, r_scale = 2, half_arc = half_arc)
        arc2_d = self.get_arc_d(self.C, self.B, D, r_scale = 2, half_arc = half_arc)
        return arc1_d, arc2_d


    def myArcs(self, half_arc=False):
        D = self.A - self.B + self.C
        arc1_d = self.get_arc_d(self.B, self.A, self.C, r_scale = 0.7, half_arc = half_arc)
        arc2_d = self.get_arc_d(D, self.A, self.C, r_scale = 0.7, half_arc = half_arc)
        return arc1_d, arc2_d
    ...
    def make_svg(self):
        ...
        for e in self.elements:
            ...
            if self.config['draw-arcs']:
                if random.random()<=self.config['altArcProb']:
                    arc1_d, arc2_d = e.myArcs(half_arc=not draw_rhombuses)
                else:
...
tiling.config['draw-tiles'] = False
saveDir = 'penrose_blobbing'
for p in [0.0, 0.25, 0.5, 0.75, 1]:
    tiling.config['altArcProb'] = p
    ...
    tiling.write_svg(f'{saveDir}/penrose_blobbing_p{p}.svg')
    cairosvg.svg2png(url=f'{saveDir}/penrose_blobbing_p{p}.svg', write_to=f'{saveDir}/penrose_blobbing_p{p}.png', scale=nGen*2)
